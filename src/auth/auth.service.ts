...
import { MODULE_MODEL, ModuleDocument } from 'src/schema/module.schema';
import { ROLE_ACCESS_MODEL, RoleAccessDocument } from 'src/schema/role_access.schema';
...
export class AuthService {
  constructor(
    ...
    @InjectModel(MODULE_MODEL) private moduleModel: Model<ModuleDocument>,
    @InjectModel(ROLE_ACCESS_MODEL) private accessModel: Model<RoleAccessDocument>
  ) { }
  ...
  async getcountries() {
    try {
      return await this.countryModel.find().select('name phoneCode');
    } catch (error) {
  ...
  async getAccessByRoleId(roleId: string) {

    const accesses = await this.accessModel.find({ roleId: new Types.ObjectId(roleId) }).lean();
    console.log('Accesses:', accesses);

    // const moduleAndAccessData = await Promise.all(
    //   accesses.map(async (access) => {
    //     const moduleData = await this.moduleModel.findById(access.module).lean();
    //     return {
    //       module: moduleData,
    //       access,
    //     };
    //   })
    // );
    // console.log(moduleAndAccessData);
